...
class UR5Controller(threading.Thread):
    ...
    def __init__(self, external_conf = None):
        ...
        log.info('UR5Controller initialized ------------------')
    ...
    def homing_procedure(self, dt, v_des, q_home, rate):
        ...
        while True:
            ...
            if (e_norm < 0.001):
                if self.gripper:
                    ur5_controller.controller_manager.gm.move_gripper(100)
...
# ----------- TALKER ----------- #
def talker(ur5_controller):
    """
    """
    # start thread
    ur5_controller.start()

    # check real robot
    if ur5_controller.real_robot:
        ur5_controller.startRealRobot()
    else:
        additional_args = ['gripper:=' + str(ur5_controller.gripper)]
                        #    'soft_gripper:=' + str(conf.robot_params[ur5_controller.robot_name]['soft_gripper'])]
                            #, 'gui:=false']
        ur5_controller.startSimulator(world_name = ur5_controller.world_name,
                                      use_torque_control = ur5_controller.use_torque_control,
                                      additional_args = additional_args)

    # specify xacro location
    xacro_path = XACRO_PATH
    ur5_controller.loadModelAndPublishers(xacro_path)
    ur5_controller.initVars()
    ur5_controller.startupProcedure()

    # sleep to avoid that the real robot crashes on the table
    time.sleep(3.)

    # loop frequency
    rate = ros.Rate(1 / conf.robot_params[ur5_controller.robot_name]['dt'])

    ur5_controller.q_des_q0 = conf.robot_params[ur5_controller.robot_name]['q_0']
    ur5_controller.q_des = np.copy(ur5_controller.q_des_q0)

    # use the point to point position controller
    if not ur5_controller.use_torque_control:
        ur5_controller.switch_controller("joint_group_pos_controller")

    # homing procedure
    if ur5_controller.homing_flag:
        if ur5_controller.real_robot:
            v_des = 0.2
        else:
            v_des = 0.6
        ur5_controller.homing_procedure(conf.robot_params[ur5_controller.robot_name]['dt'],
                                        v_des,
                                        conf.robot_params[ur5_controller.robot_name]['q_0'],
                                        rate)
    ...
    while not ros.is_shutdown():
        ur5_controller.updateKinematicsDynamics()
        ...
        if ur5_controller.real_robot:
            ur5_controller.ros_pub.add_arrow(ur5_controller.x_ee + ur5_controller.base_offset,
                                             ur5_controller.contactForceW / (6 * ur5_controller.robot.robot_mass),
                                             "green")

        # log variables
        ur5_controller.logData()
        # plot end-effector
        ur5_controller.ros_pub.add_marker(ur5_controller.x_ee + ur5_controller.base_offset)
        ur5_controller.ros_pub.publishVisual()

        # wait for syncronization of the control loop
        rate.sleep()
        ur5_controller.time = np.round(ur5_controller.time + np.array([conf.robot_params[ur5_controller.robot_name]['dt']]),  3)  # to avoid issues of dt 0.0009999
...
if __name__ == "__main__":
    ...
    try:
        talker(ur5_controller)
    except (ros.ROSInterruptException, ros.service.ServiceException):
        ros.signal_shutdown("killed")
        ur5_controller.deregister_node()
        if conf.plotting:
            ur5_controller.plotStuff()